...
def cache_clear_thumbnails():
	...
	try:
		...
		dbcur = dbcon.cursor()
		icon = dbcur.execute("SELECT cachedurl FROM texture WHERE url ='" + addonPath + "/plugin.video.onemoar/icon.png';").fetchone()
		fanart = dbcur.execute("SELECT cachedurl FROM texture WHERE url ='" + addonPath + "/plugin.video.onemoar/fanart.png';").fetchone()
		if icon is not None:
			if xbmcvfs.exists(thumbnailsPath + icon[0]):
				xbmcvfs.delete(thumbnailsPath + icon[0])
			dbcur.execute("DELETE FROM texture WHERE url ='" + addonPath + "/plugin.video.onemoar/icon.png';") #delete the icon
		if fanart is not None:
			if xbmcvfs.exists(thumbnailsPath + fanart[0]):
				xbmcvfs.delete(thumbnailsPath + fanart[0])
			dbcur.execute("DELETE FROM texture WHERE url ='" + addonPath + "/plugin.video.onemoar/fanart.png';") #delete the fanart
		dbcon.close()
		cleared = True
	except:
		from resources.lib.modules import log_utils
...
def get_connection_bookmarks():
	...
	return conn
##################
def clear_local_bookmarks(): # clear all onemoar bookmarks from kodi database
	try:
		dbcon = db.connect(get_video_database_path())
		dbcur = dbcon.cursor()
		dbcur.execute('''SELECT * FROM files WHERE strFilename LIKE "%plugin.video.onemoar%"''')
...
def update_cache_version():
	...
	try:
		...
		from resources.lib.modules import log_utils
		log_utils.log('OneMoar Addon Data Path Does not Exist. Creating Folder....', __name__, level=log_utils.LOGDEBUG)
		ad_folder = control.transPath('special://profile/addon_data/plugin.video.onemoar')
		control.makeDirs(ad_folder)
	try:
		with open(versionFile, 'r') as fh: oldVersion = fh.read()
	except: oldVersion = '0'
	try:
		curVersion = control.addon('plugin.video.onemoar').getAddonInfo('version')
...
def get_cache_version():
	...
	try:
		...
		from resources.lib.modules import log_utils
		log_utils.log('OneMoar Addon Data Path Does not Exist. Creating Folder....', __name__, log_utils.LOGDEBUG)
		ad_folder = control.transPath('special://profile/addon_data/plugin.video.onemoar')
		control.makeDirs(ad_folder)
	try:
		with open(versionFile, 'r') as fh: oldVersion = fh.read()
