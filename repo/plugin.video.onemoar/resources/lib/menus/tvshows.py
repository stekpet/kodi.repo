...
#			self.tmdb_key = ''
	...
	def search_new(self):
		...
		control.closeAll()
		control.execute('ActivateWindow(Videos,plugin://plugin.video.onemoar/?action=tvshows&url=%s,return)' % (quote_plus(url)))
	...
	def person(self, folderName=''):
		...
		control.closeAll()
		control.execute('ActivateWindow(Videos,plugin://plugin.video.onemoar/?action=tvPersons&url=%s,return)' % (quote_plus(url)))
...
#################################
	...
	def tvshowDirectory(self, items, next=True, isProgress=False, isWatched=False, folderName='OneMoar'):
...
#				nextMenu = '[COLOR skyblue]' + nextMenu + page + '[/COLOR]'
	...
	def addDirectory(self, items, queue=False, folderName='OneMoar'):
		from sys import argv # some functions like ActivateWindow() throw invalid handle less this is imported here.
		control.playlist.clear()
		if not items: # with reuselanguageinvoker on an empty directory must be loaded, do not use sys.exit()
			content = ''
		...
		try:
			...
			if self.useContainerTitles:
				try: 
					...
					homeWindow.setProperty('onemoar.tvrecent', str(getLS(40257)+' '+item.get('title')))
				except: pass
			next = ''
			for i in range(len(self.list)): self.list[i]['next'] = next
			self.worker()
		...
		try:
			...
			if self.useContainerTitles:
				try: 
					...
					homeWindow.setProperty('onemoar.tvsimilar', str(getLS(40257)+' '+item.get('title')))
				except: pass
			next = ''
			for i in range(len(self.list)): self.list[i]['next'] = next
			self.worker()
