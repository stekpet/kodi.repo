...
#		if not self.tmdb_key: self.tmdb_key = ''
		...
		self.hide_watched_in_widget = getSetting('enable.onemoarhidewatched') == 'true'
		self.useFullContext = getSetting('enable.onemoarwidgetcontext') == 'true'
	...
	def unfinishedManager(self):
		try:
			...
			del window
			if selected_items:
				refresh = 'plugin.video.onemoar' in control.infoLabel('Container.PluginName')
	...
	def search_new(self):
		...
		control.closeAll()
		control.execute('ActivateWindow(Videos,plugin://plugin.video.onemoar/?action=movies&url=%s,return)' % (quote_plus(url)))
	...
	def person(self):
		...
		control.closeAll()
		control.execute('ActivateWindow(Videos,plugin://plugin.video.onemoar/?action=moviePersons&url=%s,return)' % (quote_plus(url)))
	...
	def imdb_list(self, url, comingSoon=False, isRatinglink=False, folderName=''):
		from resources.lib.modules import log_utils
		log_utils.log('OneMoar IMDB list generation. URL: %s'% str(url), 1)
		list = []
		try:
...
#################################
	...
	def movieDirectory(self, items, unfinished=False, next=True, folderName=''):
		...
		sysaddon, syshandle = 'plugin://plugin.video.onemoar/', int(argv[1])
...
####-Context Menu and Overlays-####
				...
				cm.append(('[COLOR red]OneMoar Settings[/COLOR]', 'RunPlugin(%s?action=tools_openSettings)' % sysaddon))
####################################
				...
				if is_widget: 
					item.setProperty('isOneMoar_widget', 'true')
...
#				nextMenu = '[COLOR skyblue]' + nextMenu + page + '[/COLOR]'
	...
	def addDirectory(self, items, queue=False, folderName=''):
		...
		homeWindow.setProperty('onemoar.returnhome', returnHome)
		if not items: # with reuselanguageinvoker on an empty directory must be loaded, do not use sys.exit()
			content = ''
			control.hide()
			if is_widget != True:
	...
	def trakt_based_on_recent(self, create_directory=True, folderName=''):
		...
		try:
			...
			try: 
				...
				homeWindow.setProperty('onemoar.movierecent', str(getLS(40257)+' '+item.get('title')))
			except: pass
			next = ''
			for i in range(len(self.list)): self.list[i]['next'] = next
			self.worker()
	...
	def trakt_based_on_similar(self, create_directory=True, folderName=''):
		...
		try:
			...
			try: 
				...
				homeWindow.setProperty('onemoar.moviesimilar', str(getLS(40259)+' '+item.get('title')))
			except: pass
			next = ''
			for i in range(len(self.list)): self.list[i]['next'] = next
			self.worker()
...
#		decade = decade
	...
	def imdb_genre_list(self, url, comingSoon=False, isRatinglink=False, folderName=''):
			from resources.lib.modules import log_utils
			log_utils.log('OneMoar Genre IMDB list generation. URL: %s'% str(url), 1)
			list = []
			try:
				for i in re.findall(r'date\[(\d+)\]', url):
					url = url.replace('date[%s]' % i, (self.date_time - timedelta(days=int(i))).strftime('%Y-%m-%d'))
	...
	def getSimkl(self, url, create_directory=True, folderName=''):
		...
		try:
			...
			if u in self.simkl_link: self.list = simkl().simkl_list(url)
			if self.list is None: self.list = []
			next = ''
			for i in range(len(self.list)): self.list[i]['next'] = next
			self.worker()
	...
	def similarFromLibrary(self, tmdb=None, create_directory=True, folderName=''):
		...
		try:
			...
			all_titles = list()
			if originalMovie:
				#log_utils.log('[plugin.video.onemoar] Original Movie properties %s.'% str(originalMovie),1)
			...
			if originalMovie:
				try: 
					...
					homeWindow.setProperty('onemoar.moviesimilarlibrary', str(getLS(40257)+' '+originalMovie["title"]))
				except: pass
			if self.list is None: self.list = []
			if create_directory: self.movieDirectory(self.list, folderName=folderName)
			return self.list
