...
class RealDebridAPI:
	def __init__(self):
		self.client_ID = get_setting('affenity.rd.client_id', 'empty_setting')
		if self.client_ID in ('empty_setting', ''): self.client_ID = 'X245A4XAIBGVM'
		self.token = get_setting('affenity.rd.token', 'empty_setting')
		self.secret = get_setting('affenity.rd.secret', 'empty_setting')
		self.refresh = get_setting('affenity.rd.refresh', 'empty_setting')
		self.device_code = ''
	...
	def add_uncached(self, magnet_url, pack=False):
		...
		def _return_failed(message='Error', cancelled=False):
			...
			sleep(500)
			if cancelled:
				if confirm_dialog(text='Continue Transfer in Background?'): ok_dialog(heading='afFENity Cloud Transfer', text='Saving Result to the Real Debrid Cloud')
				else: self.delete_torrent(torrent_id)
			else: ok_dialog(heading='afFENity Cloud Transfer', text=message)
			return False
		show_busy_dialog()
		try:
		...
		if status == 'magnet_conversion':
			...
			progressDialog = progress_dialog('afFENity Cloud Transfer', icon)
