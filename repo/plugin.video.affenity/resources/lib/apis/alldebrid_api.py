...
class AllDebridAPI:
	def __init__(self):
		self.token = get_setting('affenity.ad.token', 'empty_setting')
	...
	def add_uncached(self, magnet_url, pack=False):
		def _return_failed(message='Error', cancelled=False):
			...
			sleep(500)
			if cancelled:
				if confirm_dialog(text='Continue Transfer in Background?'): ok_dialog(heading='afFENity Cloud Transfer', text='Saving Result to the All Debrid Cloud')
	...
	def clear_cache(self, clear_hashes=True):
		try:
			...
			# USER CLOUD
			try:
				dbcur.execute("""DELETE FROM maincache WHERE id=?""", ('ad_user_cloud',))
			...
			# HOSTERS
			try:
				dbcur.execute("""DELETE FROM maincache WHERE id=?""", ('ad_valid_hosts',))
