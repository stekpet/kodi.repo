...
client_id = '888228107'
user_agent = 'afFENity for Kodi'
timeout = 20.0
icon = get_icon('premiumize')

class PremiumizeAPI:
	def __init__(self):
		self.token = get_setting('affenity.pm.token', 'empty_setting')
	...
	def add_uncached(self, magnet_url, pack=False):
		...
		def _return_failed(message='Error', cancelled=False):
			...
			sleep(500)
			if cancelled:
				if confirm_dialog(heading='afFENity Cloud Transfer', text='Continue Transfer in Background?'):
					ok_dialog(heading='afFENity Cloud Transfer', text='Saving Result to the Premiumize Cloud')
