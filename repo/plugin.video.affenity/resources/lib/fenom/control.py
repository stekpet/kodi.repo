...
cacheFile = joinPath(dataPath, 'fenom_cache.db')
undesirablescacheFile = joinPath(dataPath, 'fenom_undesirables.db')
settingsFile = joinPath(dataPath, 'settings.xml')


def setting(id, fallback=None):
	try: settings_dict = jsloads(homeWindow.getProperty('fenom_settings'))
...
#			settings_dict.update(dict_item)
		...
		results = dbcur.execute('SELECT * FROM settings')
		settings = results.fetchall()
		if not settings: raise ValueError
		settings_dict = dict((k,v) for k, v, label in settings)
		settings_dict['debug.location'] = 1 if settings_dict['debug.location'] == 'true' else 0
		homeWindow.setProperty('fenom_settings', jsdumps(settings_dict))
...
SETTINGS = [
	('filter.foreign.single.audio', 'true', 'Remove identified Foreign single audio sources'),
	('filter.undesirables', 'true', 'Remove Undesirables'),
	('debug.enabled', 'false', 'Enable Debug'),
	('debug.location', 'false', 'Use Addon Log instead of Kodi Log'),
	('provider.araec', 'false', 'araec (torrentio)'),
	('provider.elfhosted', 'false', 'elfhosted (torrentio)'),
	('provider.torrentio', 'true', 'torrentio (pack capable)'),
	('provider.bitsearch', 'true', 'bitsearch (pack capable)'),
	('bitsearch.sort', 'false', 'bitsearch (pack capable)'),
	('provider.nyaa', 'false', 'nyaa'),
	('provider.piratebay', 'false', 'piratebay (pack capable)'),
	('provider.torrentdownload', 'false', 'torrentdownload (pack capable)'),
	('provider.library', 'false', 'library')
]

def getProviderDefaults():
	return dict((k,v) for k, v, label in SETTINGS if k.startswith('provider.'))
