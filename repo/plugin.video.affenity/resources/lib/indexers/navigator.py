...
download_directory, easynews_active, get_icon, unquote = s.download_directory, s.easynews_active, k.get_icon, k.unquote
...
get_main_lists = nc.get_main_lists
...
class Navigator:
	def __init__(self, params):
		...
		self.category_name = self.params_get('name', 'afFENity')
	...
	def premium(self):
		...
		if 'Real-Debrid' in debrids: self.add({'mode': 'navigator.real_debrid'}, 'Real Debrid', 'realdebrid')
		if 'Premiumize.me' in debrids: self.add({'mode': 'navigator.premiumize'}, 'Premiumize', 'premiumize')
		if 'AllDebrid' in debrids: self.add({'mode': 'navigator.alldebrid'}, 'All Debrid', 'alldebrid')
		if easynews: self.add({'mode': 'navigator.easynews'}, 'Easynews', 'easynews')
		self.end_directory()
	...
	def my_content(self):
		if get_setting('affenity.trakt.user', 'empty_setting') not in ('empty_setting', ''):
	...
	def tools(self):
		...
		self.add({'mode': 'navigator.scrapers_settings'}, 'Scrapers Settings', 'settings')
		...
		self.add({'mode': 'restart_services', 'isFolder': 'false'}, 'Restart afFENity Services (ADVANCED!!)', 'settings2')
		self.add({'mode': 'toggle_language_invoker', 'isFolder': 'false'}, 'Toggle Language Invoker (ADVANCED!!)', 'settings2')
		self.end_directory()

	def scrapers_settings(self):
		self.add({'mode': 'filters_select', 'isFolder': 'false'}, 'Enable/Disable Filters', 'settings')
		self.add({'mode': 'undesirables_input', 'isFolder': 'false'}, 'Input Undesirables', 'settings')
		self.add({'mode': 'undesirables_remove', 'isFolder': 'false'}, 'Remove Undesirables', 'settings')
		self.add({'mode': 'providers_select', 'isFolder': 'false'}, 'Enable/Disable Providers', 'settings')
		self.add({'mode': 'debug_select', 'isFolder': 'false'}, 'Debug Options', 'settings')
		self.end_directory()

	def maintenance(self):
		self.add({'mode': 'database_integrity_check_cache', 'isFolder': 'false'}, 'Check for Corrupt Databases', 'settings')
	...
	def changelog_utils(self):
		affenity_clogpath = tp('special://home/addons/plugin.video.affenity/resources/text/changelog.txt')
		self.add({'mode': 'show_text', 'heading': 'Changelog', 'file': affenity_clogpath, 'font_size': 'large', 'isFolder': 'false'}, 'Changelog', 'lists', False)
	...
	def search_history(self):
		...
		def _builder():
			for i in data:
				...
				if '|' in key_id: display = '%s (%s)' % tuple(key_id.split('|'))
				else: display = key_id
	...
	def exit_media_menu(self):
		params = get_property('affenity.exit_params')
		if params: return container_refresh_input(params)

	def tips(self):
		tips_location = 'special://home/addons/plugin.video.affenity/resources/text/tips'
