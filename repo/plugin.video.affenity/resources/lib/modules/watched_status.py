...
watched_indicators_function, lists_sort_order = settings.watched_indicators, settings.lists_sort_order
date_offset = settings.date_offset
indicators_dict = {0: 'watched_db', 1: 'trakt_db'}
progress_db_string = 'affenity_hidden_progress_items'
...
def get_database(watched_indicators=None):
	if watched_indicators == None: watched_indicators = watched_indicators_function()
	return indicators_dict[watched_indicators]

def make_database_connection(database_file):
	return connect_database(database_file)
...
def clear_local_bookmarks():
	try:
		...
		file_ids = dbcon.execute("SELECT idFile FROM files WHERE strFilename LIKE 'plugin.video.affenity%'").fetchall()
