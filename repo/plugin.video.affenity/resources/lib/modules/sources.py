...
from modules.player import afFENityPlayer
...
class Sources():
	...
	def playback_prep(self, params=None):
		...
		self.folders_ignore_filters = get_setting('affenity.results.folders_ignore_filters', 'false') == 'true'
#		self.filter_size = get_setting('affenity.results.filter_size') == 'true'
		self.filter_size = get_setting('affenity.results.filter_size')
		...
		self.include_unknown_size = get_setting('affenity.results.include.unknown.size', 'false') == 'true'
		self.include_3D_results = get_setting('affenity.include_3d_results', 'true') == 'true'
		self._update_meta()
		self._search_info()
	...
	def filter_results(self, results):
		...
		if   self.filter_size == '1':
			...
			max_size = string_to_float(get_setting('affenity.results.file_size', '10000'), '10000') / 1000
			...
			max_size = ((0.125 * (0.90 * string_to_float(get_setting('affenity.results.line_speed', '20'), '20'))) * duration)/1000
			results = [i for i in results if i['scrape_provider'] == 'folders' or min_size <= i['size'] <= max_size]
...
#		self.ext_folder, self.ext_name = external_scraper_info()
		self.ext_folder, self.ext_name = 'plugin.video.affenity', 'fenom'
...
#			self.provider_defaults = [k.split('.')[1] for k, v in manual_function_import('%s.modules.control' % self.ext_name, 'getProviderDefaults')().items() if v == 'true']
			self.ext_sources = manual_module_import('%s.sourcesdir' % self.ext_name)
			self.provider_defaults = [k.split('.')[1] for k, v in manual_function_import('%s.control' % self.ext_name, 'getProviderDefaults')().items() if v == 'true']
		except: return False
		return True
...
#			sourceDict = [(i, manual_function_import('%s.sources_%s.%s.%s' % (self.ext_name, self.ext_name, 'torrents', i), 'source')) for i in active_sources]
			sourceDict = [(i, manual_function_import('%s.sourcesdir.%s.%s' % (self.ext_name, 'torrents', i), 'source')) for i in active_sources]
	...
	def get_folderscraper_info(self):
		folder_info = [(get_setting('affenity.%s.display_name' % i), i, source_folders_directory(self.media_type, i)) for i in folder_scrapers]
	...
	def _special_filter(self, results, key, enable_setting):
		if key == self.hevc_filter_key and enable_setting in (0,2):
			hevc_max_quality = self._get_quality_rank(get_setting('affenity.filter_hevc.%s' % ('max_autoplay_quality' if self.autoplay else 'max_quality'), '4K'))
	...
	def debridPacks(self, debrid_provider, name, magnet_url, info_hash, download=False):
		...
		self._kill_progress_dialog()
		return afFENityPlayer().run(link, 'video')
