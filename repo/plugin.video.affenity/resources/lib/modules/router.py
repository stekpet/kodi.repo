...
def routing(sys):
	...
	if 'playback.' in mode:
		...
		if mode == 'playback.video':
			...
			return afFENityPlayer().run(_get('url', None), _get('obj', None))
	if 'choice' in mode:
	...
	if '_cache' in mode:
		...
		if mode == 'database_integrity_check_cache':
			return base_cache.database_integrity_check()
	if '_image' in mode:
	...
	if mode == 'person_direct.search':
		...
		return person_direct_search(_get('key_id'))
	...
	##Scrapers modes###
	if mode == 'filters_select':
		from fenom.settings import filtersSelect
		filtersSelect()
	if mode == 'debug_select':
		from fenom.settings import debugSelect
		debugSelect()
	if mode == 'providers_select':
		from fenom.settings import providersSelect
		providersSelect()
	if mode == 'undesirables_input':
		from caches.undesirables_cache import undesirablesInput
		undesirablesInput()
	if mode == 'undesirables_remove':
